 ...
 */
export const batchVerify = (values, verifys, regsExt = {}, hideToast = false, getKey = false) => {
  for (const key in verifys) {
    if (verifys.hasOwnProperty(key) && values[key] !== undefined && verify(values[key], verifys[key], regsExt, hideToast) !== true) {
